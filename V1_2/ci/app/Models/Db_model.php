...
class Db_model extends Model
{
    ...
    ==================================================================*/
    //Récupère toutes les candiatures d'un concours en particulier
    public function get_all_candidats()
    {
        $requete = "SELECT  * from  t_candidature_can where     can_id = 1;  select     * from    t_candidature_can where    can_code = 'AQSwdze4';";
        $resultat = $this->db->query($requete);
        return $resultat->getResultArray();
    }
...
//remettre le lien symbolique après avoir changer de version
    ...
    //     //Récuparation (+ traitement si nécessaire) des données du formulaire
    //     $code_candidature = $saisie['code_candidature'];
    //     $code_dossier = $saisie['code_dossier'];
    //     $sql = "INSERT INTO t_compte_cpt VALUES('" . $login . "@Olympix.com', '" . $mot_de_passe . "', '" . $nom . "', '" . $prenom . "', 'A');";
